...
int VLC_test() {
	...
	// Initialise libVLC.
	libvlc2 = libvlc_new(vlc_argc, vlc_argv);
	libvlc = libvlc_new(vlc_argc, vlc_argv);
	libvlc_release(libvlc2);
	if (NULL == libvlc) {
		printf("LibVLC initialization failure.\n");
		return EXIT_FAILURE;
	}

	m = libvlc_media_new_path(libvlc, "Z:\\cpp\\visual_studio\\GROUGROUengine\\video\\exemple.mkv");
	mp = libvlc_media_player_new_from_media(m);
	libvlc_media_release(m);
	unsigned int _w=0, _h=0;
	libvlc_video_get_size(mp, 1, &_w, &_h);

	std::cout <<  "-" << _w << "-" << _h << "-" << std::endl;
	void* salop = &context;
	libvlc_video_set_callbacks(mp, lock, unlock, display, &context);
	libvlc_video_set_format(mp, "RV16", VIDEOWIDTH, VIDEOHEIGHT, VIDEOWIDTH * 2);
	std::cout << "VAL=="<<libvlc_media_player_play(mp) << std::endl;
